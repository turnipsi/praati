...
This software has been written by Juha Erkkilä <je@turnipsi.no-ip.org>.
Patches (portability, security, features, whatever) are mostly
welcome.

=head2 Installation instructions

Here are installation instructions for OpenBSD 6.2 (IN PROGRESS).

Package dependencies for runtime, install these packages:
...
Setup C</etc/httpd.conf> like this:
    ...
    server "praati.example.com" {
	    listen on * tls port 443
	    root "/praati-static"	# optional

	    tls certificate "/etc/ssl/praati/praati.example.com.crt"
	    tls key "/etc/ssl/praati/praati.example.com.key"
